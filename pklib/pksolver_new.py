...
class PK_solver():
    ...
    def create_C_br(self, cls_probs, uconf=0.):
        ...
        # Iterate over all pairs of classes where the first has a higher probability than the second
        for i in range(self.C - 1):
            for j in range(i + 1, self.C):
                # Create a constraint for each valid pair
                self.C_br.append((idx[i], idx[j], (1-uconf) * np.round((cls_probs[idx[i]] - cls_probs[idx[j]]) * self.N)))
    ...
    def create_C_br_noisy(self, cls_probs, uconf=0., noise=0.):
        ...
        self.C_br = [(idx[i], idx[j], 0) for i in range(C) for j in range(i + 1, C)]
    ...
    def create_C_br_partial(self, cls_probs, uconf=0., N=10):
        idx = np.argsort(-cls_probs)  # Sort classes by descending probabilities
    # Generate all possible pairs of classes where the first has a higher probability than the second
        all_pairs = [(idx[i], idx[j], (1-uconf) * np.round((cls_probs[idx[i]] - cls_probs[idx[j]]) * self.N))
                 for i in range(len(cls_probs)) for j in range(i + 1, len(cls_probs))]
    # Select the top N pairs based on the difference in probabilities (adjusted by uconf)
    # Ensure N does not exceed the total number of pairs
        N = min(N, len(all_pairs))
        self.C_br = sorted(all_pairs, key=lambda x: x[2], reverse=True)[:N]
    ...
    # Generate all possible pairs of classes where the first has a higher probability than the second
        all_pairs = [(idx[i], idx[j], (1-uconf) * np.round((cls_probs[idx[i]] - cls_probs[idx[j]]) * self.N))
                 for i in range(len(cls_probs)) for j in range(i + 1, len(cls_probs))]
    # Select the bottom N pairs based on the difference in probabilities (adjusted by uconf)
    # Ensure N does not exceed the total number of pairs
        N = min(N, len(all_pairs))
        self.C_br = sorted(all_pairs, key=lambda x: x[2])[:N]
    ...
    def create_C_br_partial_rand(self, cls_probs, uconf=0., N=10):
        ...
        all_pairs = [(idx[i], idx[j], (1-uconf) * np.round((cls_probs[idx[i]] - cls_probs[idx[j]]) * self.N))
                 for i in range(len(cls_probs)) for j in range(i + 1, len(cls_probs))]
    # Randomly select N pairs to create constraints, ensuring N does not exceed the total number of pairs
        N = min(N, len(all_pairs))
        self.C_br = random.sample(all_pairs, k=N)
