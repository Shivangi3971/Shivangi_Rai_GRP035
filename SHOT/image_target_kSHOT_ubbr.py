...
def data_load(args): 
    ...
    dsets["target"] = ImageList_idx(txt_tar, root="../data/{}/train/".format(args.dset), transform=image_train())
    dset_loaders["target"] = DataLoader(dsets["target"], batch_size=train_bs, shuffle=True, num_workers=args.worker, drop_last=False)

    dsets["test"] = ImageList_idx(txt_test, root="../data/{}/train/".format(args.dset), transform=image_test())
    dset_loaders["test"] = DataLoader(dsets["test"], batch_size=train_bs*3, shuffle=False, num_workers=args.worker, drop_last=False)
    dsets["valid"] = ImageList_idx(txt_tar, root="../data/{}/validation/".format(args.dset), transform=image_train() if args.use_train_transform else image_test())
...
if __name__ == "__main__":
    ...
    if args.dset == 'office31':
        ...
        args.class_num = 31
    if args.dset == 'visda-2017':
        args.names = ['aeroplane', 'bicycle', 'bus','car', 'horse', 'knife', 'motorcycle', 'person', 'plant', 'skateboard', 'train', 'truck']
        #['train', 'validation']
        args.class_num = 12
    ...
    for t in args.names:
        ...
        args.t_dset_path = folder + args.dset + '/validation/' + 'image_list.txt'
        args.test_dset_path = folder + args.dset + '/validation/' + 'image_list.txt'
